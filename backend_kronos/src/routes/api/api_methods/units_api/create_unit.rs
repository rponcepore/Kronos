...
pub async fn create_unit(valid_req: Json<KronosRequest>) -> Result<KronosResponse, KronosApiError> {
    ...
    let unit_request = match &valid_req.unit_request {
        ...
        None => {
            return Err(KronosApiError::BadRequest(
                "Request to create new unit did not include a valid unit_request object/field."
                    .to_string(),
            ))
        }
    };
...
async fn add_unit_to_db(
    unit_request: &ValidUnitRequest,
...
async fn check_valid_create_unit_request(
    req: &UnitRequest,
) -> Result<ValidUnitRequest, KronosApiError> {
    let uic = match req.uic.clone() {
        Some(uic) => uic,
        None => return Err(KronosApiError::BadRequest(
            "Request to create a new unit did not include a valid uic in the unit_request object."
                .to_string(),
        )),
    };
    ...
    let possible_uic = match Unit::find_by_id(&uic).one(&db).await {
        Ok(possible_uic) => match possible_uic {
            Some(possible_uic) => {
                return Err(KronosApiError::BadRequest(format!(
                    "The UIC {} already exists.",
                    uic
                )));
            }
            None => {} //base case
    ...
    */
    ...
    match &req.component {
        Some(component) => match component.as_str() {
            ...
            _ => {
                return Err(KronosApiError::BadRequest(
                    "Component must be either ACTIVE, RESERVE, or GUARD.".to_string(),
                ))
            }
        },
        None => {
...
fn require_non_empty_field(
    ...
    let stringval = match value {
        ...
        None => {
            return Err(KronosApiError::BadRequest(format!(
                "{field_name} cannot be null. You must provide a value."
            )))
        }
    };
