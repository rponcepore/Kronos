...
pub async fn assemble_paragraph_summary(paragraph: &paragraph::Model, db: &DatabaseConnection) -> Result<ParagraphSummary, DbErr> {
        ...
        _ => {
            ...
            for subparagraph in result{
                let subparagraph_summary = Box::pin(async move {
                    assemble_paragraph_summary(&subparagraph, db)
                    .await
                }).await?;
